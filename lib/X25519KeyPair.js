...
const ready = promisify(ed25519_to_x25519.ready);
...
export class X25519KeyPair extends LDKeyPair {
  ...
  static async fromEdKeyPair(edKeyPair) {
    const xKey = new X25519KeyPair({
      controller: edKeyPair.controller
    });

    xKey.publicKeyBase58 = await X25519KeyPair
   ...
   */
  static async convertFromEdPublicKey(edPublicKeyBase58) {
    ...
    // FIXME: there should be check to ensure WASM is ready?
    // ed25519_to_x25519 has a `ready` helper with a callback for this purpose
    // is this still required in modern nodejs?
    try {
      await ready();
    } catch(e) {
      // ready callback is not compatible with promisify so ready is indicated
      // by it throwing here
      console.log('wasm ready');
    }

    const dhPubkeyBytes = ed25519_to_x25519.convert_public_key(edPubkeyBytes);
