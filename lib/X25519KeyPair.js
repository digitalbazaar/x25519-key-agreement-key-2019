...
class X25519KeyPair extends LDKeyPair {
  ...
  static fromFingerprint({fingerprint}) {
    ...
    // buffer is: 0x25 <public key bytes>
    if(buffer[0] !== 0x25) {
      throw new Error(`Unsupported Fingerprint Type: ${fingerprint}`);
    }

    return new X25519KeyPair({
      publicKeyBase58: base58.encode(buffer.slice(1))
    });
  }
  ...
  verifyFingerprint(fingerprint) {
    ...
    // validate the first multicodec byte 0x25
    const valid = fingerprintBuffer.slice(0, 1).toString('hex') === '25' &&
      publicKeyBuffer.equals(fingerprintBuffer.slice(1));
    if(!valid) {
      return {
